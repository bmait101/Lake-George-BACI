...
```{r loadData}
filename <- "LakeGeorge_historical_CO_MMI.xlsx" # data file must be saved to same location as code for this to work

#----------------------------------------------------------------------------------
# Load sample data
macro_sampleData <- read_excel(filename, sheet = "Site & sample info")
...
#----------------------------------------------------------------------------------
# Load multi-metric index data (filter for CO MMI v5)
macro_indices <- read_excel(filename, sheet = "Index results - CO_MMI_v5")

#----------------------------------------------------------------------------------
# Load metrics data
macro_metrics <- read_excel(filename, sheet = "Metrics")
...
macro_data_filtered %>% 
  ...
  geom_text(aes(x = "Downstream", y = 54), label = "High-Scoring", size = 3, color = "black", nudge_x = 0.4)
```
What is the percentage increase in MMI at Impact sites following the dam removal?
```{r}
macro_data_filtered %>% 
       filter(Control_Impact == "Impact", Before_After == "Before") %>% 
       summarise(Mean_score = mean(`Index Score`))

macro_data_filtered %>% 
       filter(Control_Impact == "Impact", Before_After == "After") %>% 
       summarise(Mean_score = mean(`Index Score`))
```
...
# Density of tolerant/intolerance taxa BACI test
Does the density of tolerant and intolerant taxa differ between control and impact reaches? Is so, does the difference persist after the restoration?

First, check response variable for normality:

```{r}
qqnorm(macro_data_filtered$`Density - Intolerant Taxa`)
qqline(macro_data_filtered$`Density - Intolerant Taxa`)
hist(macro_data_filtered$`Density - Intolerant Taxa`)
```
Density is not normally distributed, so we will need to use a log transformation.
...
```{r}
Tolerant_density_glmm <- glmmTMB(log(`Density - Intolerant Taxa`) ~ Before_After * Control_Impact + (1|Reach),
                         data = macro_data_filtered,
                         family = Gamma(link="log"))

summary(Tolerant_density_glmm)

macro_data_filtered %>% 
  ggplot(aes(x = Control_Impact, y = log(`Density - Intolerant Taxa`), color = Before_After)) +
  geom_boxplot() +
  theme_classic() +
  labs(y = "log Density Intolerant Taxa (inds/m^2)")
```
A significant interaction between Before_After and Control_Impact indicates that density of intolerant taxa at the impact sites significantly increased after the restoration relative to the control sites.
...
macro_taxaMatrix <- read_excel(filename,sheet = "Taxa matrix")
