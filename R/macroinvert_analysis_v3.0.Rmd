...
Consider visualizing MMI change over time. Or choose another metric.
```{r message=FALSE, warning=FALSE}
# Create a summary table
MMI_over_time_stats <- macro_data_filtered %>% 
  group_by(Reach,
           Sample) %>% 
  summarise(Mean_MMI = mean(`Index Score`, na.rm = T),
            SD = sd(`Index Score`, na.rm = T))
  
MMI_over_time_stats %>% 
  ggplot(aes(x = Sample, y = Mean_MMI, color = Reach, group = Reach)) +
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin = Mean_MMI - SD, ymax = Mean_MMI + SD),
                width=0.2,
...
# plot for Charlie's Lake George tour presentation
macro_data_filtered %>%
  ggplot(aes(x = Reach, y = `Index Score`, color = Sample)) +
  geom_boxplot() +
...
ggsave(filename = "LakeGeorge_MMIplot_3.12.2026.png",
       width = 7,
       height = 5,
       scale = 0.8,
       units = "in",
...
T-Tests to test for differences in means between groups:
```{r}
...
# plot settings for Charlie's field trip presentation:
ggplot() +
  # plot communities
  geom_point(data = nmds_communityScores,
...
# Save plot
ggsave(filename = "LakeGeorge_NMDSplot_3.12.2026.png",
       width = 7,
       height = 5,
       scale = 0.8,
       units = "in",
